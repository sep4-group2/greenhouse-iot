...
{
  fake_wifiModule_send("OK\r\n", 5);
  TEST_ASSERT_EQUAL(WIFI_OK, wifi_command_create_TCP_connection("The IP adress", 8000, NULL, NULL, &received_length));
  fake_wifiModule_send("OK\r\n", 5);
  TEST_ASSERT_EQUAL(WIFI_OK, wifi_command_create_TCP_connection("The IP adress", 8000, TCP_Received_callback_func, TEST_BUFFER, &received_length));
}

void test_wifi_TCP_connection_failed()
{
  fake_wifiModule_send("FAIL\r\n", 5);
  TEST_ASSERT_EQUAL(WIFI_FAIL, wifi_command_create_TCP_connection("The IP adress", 8000, NULL, NULL, &received_length));
}
...
void test_wifi_send_data_with_a_zero(){
  fake_wifiModule_send("OK\r\n", 5);
  TEST_ASSERT_EQUAL(WIFI_OK,wifi_command_TCP_transmit((uint8_t *)"sendTh\0s!A!",11));
  TEST_ASSERT_EQUAL_STRING("AT+CIPSEND=11\r\n",uart_send_string_blocking_fake.arg1_val );
  TEST_ASSERT_EQUAL_STRING("sendTh\0s!A!",(char *)uart_send_array_blocking_fake.arg1_val );
